...
export async function addToCart(req, res) {
  ...
  try {
    if (!req.session.userId) {
      return res.status(401).json({ message: "You must be logged in to ad items to your cart" });
    }
    ...
    const getItemDetails = await db.get(`SELECT * FROM products WHERE id = ?`, [productId]);

    if (!getItemDetails) {
      return res.status(404).json({ message: "No product with that id exists" });
    }
